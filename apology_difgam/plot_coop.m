...
%  TRE=TREv{iTRE};
  ...
  for ivari=1:length(idd)
    ...
    for igam=1:length(gamv)
      ...
      inpf=['data/SD_eAF' eAF '_g' gam '_b' b '_e' e '_epsi' epsi '_delta' delta];
      isubp=isubp+1;
      subplot(nxsubp,nysubp,isubp);
      if (idd(ivari)~=2 && idd(ivari)~=3)  plot_param1(inpf,xcol,xlab,tit);
...
for iTRE=1:length(TREv)
  ...
  hold all;
  for i=1:3 %4
    h=plot(INPM.data(:,xcol),INPM.data(:,nS-4+i),[TREcol{iTRE} coolin{i}]);
    set(h,'MarkerSize',2);
  end
...
for iTRE=1:length(TREv)
  ...
  hold all;
  for i=1:3 %4
    h=semilogx(INPM.data(:,xcol),INPM.data(:,nS-4+i),[TREcol{iTRE} coolin{i}]);
    set(gca,'xscale','log');
    set(h,'MarkerSize',2);
  end
