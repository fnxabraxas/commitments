...
TREv={'11'}; %{'00'; '01'; '10'; '11'};
TRElabv={'NC'} %{'strict'; 'antibasic'; 'basic'; 'relax'};
...
for iTRE=1:length(TREv)
  ...
  for ivari=1:length(variv)
    ...
    for idelta=1:length(deltav)
      ...
      if ivari==1  tit=['$\alpha$=' e ];
      elseif ivari==2  tit=['b=' b ];
      elseif ivari==3  tit=['b=' b ' error=' e ' delta=' delta];
      elseif ivari==4  tit=['b=' b ' error=' e ' epsilon=' epsi]; end  
      
      inpf=['data/SD_b' b '_e' e '_epsi' epsi '_delta' delta '_' TRE '.dat']
      isubp=isubp+1;
