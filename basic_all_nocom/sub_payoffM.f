...
c	TRE: 00 (strict), 10 (basic), 01(antibasic), 11(relax)
	...
	elseif (com.eq.1) then
	  ...
	  elseif((Pin(is1).eq.0).and.(Pin(is2).eq.0)) then
	    ...
	    Wd1=b*e-c*(1.d0-e)+delta*(1.d0-2.d0*e)
	    wd2=b*(1.d0-e)-c*e-delta*(1.d0-2.d0*e)
	    coopd1=1.d0-e
	    coopd2=e
	    ccd=e*(1.d0-e)
	  ...
	  pay1=(1.d0-w)*(pay1+(b-c)*(nRC-1.d0)+Wd1) 
	  pay2=(1.d0-w)*(pay2+(b-c)*(nRC-1.d0)+Wd2)

	  coop1=(1.d0-w)*((nRC-1.d0)+coopd1)
	  coop2=(1.d0-w)*((nRC-1.d0)+coopd2)
	  def1=0.d0
	  def2=0.d0
	  ccm=(1.d0-w)*((nRC-1.d0)+ccd)
