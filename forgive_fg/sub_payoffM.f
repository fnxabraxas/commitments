...
c	TRE: 00 (strict), 10 (basic), 01(antibasic), 11(relax)
	...
	elseif (com.eq.1) then
	  if((Pin(is1).eq.1).and.(Pin(is2).eq.1)) then
	    ga=(1.d0-e)**2.d0+f(is2)*f(is1)*e**2
     +				+e*(1.d0-e)*(f(is1)+f(is2))
	    ha1=(b-c)*(1.d0-e)**2.d0+e*(1.d0-e)*(b*f(is2)-c*f(is1))
	    ha2=(b-c)*(1.d0-e)**2.d0+e*(1.d0-e)*(-c*f(is2)+b*f(is1))
	    hca1=(1.d0-e)**2.d0+e*(1.d0-e)*f(is1)
	    hca2=(1.d0-e)**2.d0+e*(1.d0-e)*f(is2)
	    hcca=(1.d0-e)**2.d0
	    Wd1=(b-c)*(1.d0-e) -delta*e*(f(is1)-f(is2)) -ha1*w
	    wd2=(b-c)*(1.d0-e) +delta*e*(f(is1)-f(is2)) -ha2*w
	    coopd1=((1.d0-e)**2.d0)*(1.d0-w) + e*(1.d0-e)
	    coopd2=coopd1
	    ccd=((1.d0-e)**2.d0)*(1.d0-w)
	  elseif((Pin(is1).eq.0).and.(Pin(is2).eq.0)) then
	    ga=e**2.d0+f(is2)*f(is1)*(1.d0-e)**2
     +				+e*(1.d0-e)*(f(is1)+f(is2))
	    ha1=(b-c)*e**2.d0+e*(1.d0-e)*(b*f(is2)-c*f(is1))
	    ha2=(b-c)*e**2.d0+e*(1.d0-e)*(-c*f(is2)+b*f(is1))
	    hca1=e**2.d0+e*(1.d0-e)*f(is1)
	    hca2=e**2.d0+e*(1.d0-e)*f(is2)
	    hcca=e**2.d0
	    Wd1=(b-c)*e -delta*(1.d0-e)*(f(is1)-f(is2)) -ha1*w
	    wd2=(b-c)*e +delta*(1.d0-e)*(f(is1)-f(is2)) -ha2*w
	    coopd1=(e**2.d0)*(1.d0-w) + e*(1.d0-e)
	    coopd2=coopd1
	    ccd=(e**2.d0)*(1.d0-w)
	  else
	    ga=f(is2)*e**2.d0+f(is1)*(1.d0-e)**2
     +				+e*(1.d0-e)*(1.d0+f(is1)*f(is2))
	    ha1=e*(1.d0-e)*(b-c)+b*f(is2)*e**2.d0-c*f(is1)*(1.d0-e)**2.d0
	    ha2=e*(1.d0-e)*(b-c)-c*f(is2)*e**2.d0+b*f(is1)*(1.d0-e)**2.d0
	    hca1=e*(1.d0-e)+f(is1)*(1.d0-e)**2.d0
	    hca2=e*(1.d0-e)+f(is2)*e**2.d0
	    hcca=e*(1.d0-e)
	    Wd1=b*e-c*(1.d0-e)
     +		+delta*((1.d0-e)*(1.d0-f(is1))-e*(1.d0-f(is2))) -ha1*w
	    wd2=b*(1.d0-e)-c*e
     +		-delta*((1.d0-e)*(1.d0-f(is1))-e*(1.d0-f(is2))) -ha2*w
	    coopd1=e*(1.d0-e)*(1.d0-w) + (1.d0-e)**2
	    coopd2=e*(1.d0-e)*(1.d0-w) + e**2
	    ccd=e*(1.d0-e)*(1.d0-w)
	      if((Pin(is1).eq.0).and.(Pin(is2).eq.1)) then
	    	ga=f(is1)*e**2.d0+f(is2)*(1.d0-e)**2
     +				+e*(1.d0-e)*(1.d0+f(is1)*f(is2))
	    	ha1=e*(1.d0-e)*(b-c)
     +			-c*f(is1)*e**2.d0+b*f(is2)*(1.d0-e)**2.d0
	    	ha2=e*(1.d0-e)*(b-c)
     +			+b*f(is1)*e**2.d0-c*f(is2)*(1.d0-e)**2.d0
	    	hca1=e*(1.d0-e)+f(is1)*e**2.d0
	    	hca2=e*(1.d0-e)+f(is2)*(1.d0-e)**2.d0
	    	hcca=e*(1.d0-e)
	    	Wd1=-c*e+b*(1.d0-e)
     +		  -delta*((1.d0-e)*(1.d0-f(is2))-e*(1.d0-f(is1))) -ha1*w
	    	wd2=-c*(1.d0-e)+b*e
     +		  +delta*((1.d0-e)*(1.d0-f(is2))-e*(1.d0-f(is1))) -ha2*w
