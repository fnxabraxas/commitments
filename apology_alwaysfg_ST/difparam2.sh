...
for igam in ${!gamv[*]}; do for ieAF in ${!eAFv[*]}; do for ib in ${!bv[*]}; do for ie in ${!ev[*]}; do for iepsi in ${!epsiv[*]}; do for idelta in ${!deltav[*]}; do
gam=${gamv[$igam]}; eAF=${eAFv[$ieAF]}; b=${bv[$ib]}; e=${ev[$ie]}; epsi=${epsiv[$iepsi]}; delta=${deltav[$idelta]}

lab="eAF"$eAF"_g"$gam"_S"$S"_T"$T"_e"$e"_epsi"$epsi"_delta"$delta
