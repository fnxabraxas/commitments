...
for iTRE=1:length(TREv)
  ...
  for ivari=1:length(idd)
    ...
    outf=[xlabf '_' TRElabv{iTRE} '_for00'];
    filenameps=[outf '.ps'];
...
if TRE=='00'
  ST=[1+4 2+4 10-6+4 12-6+4 14-6+4  1+14 2+14 10-6+14 12-6+14 14-6+14  2 ];
  labST={'(P,C,-,F,NA)';  '(P,D,-,F,NA)'; '(A,C,AllD,F,NA)'; '(A,C,TFT,F,NA)'; '(A,D,AllD,F,NA)'; '(P,C,-,F,A)'; '(P,D,-,F,A)'; '(A,C,AllD,F,A)'; '(A,C,TFT,F,A)'; '(A,D,AllD,F,A)' ;'(NC,-,AllD,-,-)'};
  cST=['b' 'y' 'r'  'c' 'm'  'b' 'y' 'r'  'c' 'm'   'k' ];
  lST={'-' '-' '-' '-' '-' '--' '--' '--' '--' '--' '-' };
else
  ST=[2+4 4+4 6+4 10+4 12+4 14+4 2+20 4+20  6+20 10+20 12+20 14+20    2 ];
  labST={'(P,C,AllD,F,NA)'; '(P,C,TFT,F,NA)';  '(P,D,AllD,F,NA)'; '(A,C,AllD,F,NA)'; '(A,C,TFT,F,NA)'; '(A,D,AllD,F,NA)';'(P,C,AllD,F,A)'; '(P,C,TFT,F,A)';   '(P,D,AllD,F,A)'; '(A,C,AllD,F,A)'; '(A,C,TFT,F,A)'; '(A,D,AllD,F,A)'; '(NC,-,AllD,-,-)'  };
  cST=['b' 'g' 'y' 'r' 'c' 'm'  'b' 'g' 'y' 'r' 'c' 'm'   'k' ];
  lST={'-' '-' '-' '-' '-' '-' '--' '--' '--' '--' '--' '--'    '-' };
end
[inf]
